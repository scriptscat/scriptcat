...
// install.sh/uninstall.sh previously had zero behavioral test coverage — only manifest-gen.ts's
// pure function was tested, never the scripts themselves. Driven here as real subprocesses
// against a fake HOME, exactly like a maintainer would run them manually to smoke-test a release,
// but automated and isolated. install.ps1/uninstall.ps1 aren't covered here — there is no
...
describe.skipIf(process.platform === "win32" || !existsSync(distHostJs))("install.sh / uninstall.sh", () => {
  let home: string;
  ...
  it("升级（安装比已记录版本更新的版本）会把旧版本记为 previous；--rollback 据此还原 manifest 并保留新版本目录", async () => {
    ...
    // The newer version's install dir is never deleted by a rollback — only the manifest
    // registration and install-metadata.json pointer move back to the previous version.
    expect(existsSync(newInstallDir)).toBe(true);
  });
});
