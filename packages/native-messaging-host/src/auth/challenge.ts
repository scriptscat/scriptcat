...
// HMAC-SHA-256 challenge-response. The host only ever stores tokenHash = SHA-256(token) — never
// the raw token — so the MAC is keyed on tokenHash rather than the raw token itself:
// `HMAC_SHA256(key=tokenHash, nonce + "|" + endpointName)`. This is the only construction that
// lets the host verify a session using solely what it's allowed to persist. Binding the
// endpoint name into the MAC prevents replaying a captured response against a different
// socket/pipe.
