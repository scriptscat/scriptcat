...
describe("isValidExtensionId - 严格校验扩展 ID（doc 05 §5, doc 06 §5）", () => {
  it("32 个 a-p 字符的 ID 合法", () => {
    expect(isValidExtensionId(VALID_ID)).toBe(true);
  });

  it("prelim 默认值 fomrtutthjerocmw 非法（不是有效的 32 位 a-p ID）", () => {
...
describe("generateManifest - 类型化 manifest 生成（doc 05 §5 Solution A）", () => {
...
describe("serializeManifest - 无 BOM 的 UTF-8 输出（doc 06 §5）", () => {
  it("序列化结果不含 BOM 字符", () => {
    const result = generateManifest({ extensionIds: [VALID_ID], hostExecutablePath: "/bin/host" });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
