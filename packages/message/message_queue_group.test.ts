...
describe("MessageQueueGroup", () => {
  ...
  describe("中间件功能测试", () => {
    ...
    it.concurrent("应该支持异步中间件", async () => {
      ...
      group.subscribe("test-middlewareAsync", handler);
      group.emit("test-middlewareAsync", { data: "test-middlewareAsync" });
      ...
      expect(handler).toHaveBeenCalledWith({ data: "test-middlewareAsync" });
    });
  });
