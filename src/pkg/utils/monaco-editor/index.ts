...
const langPromise = Promise.resolve(); // 留位用。日後再處理
...
langPromise.then(() => updateLang(localStorage.language));
...
export function registerEditor() {
  ...
  Promise.all(["{}", "{}"]).then(
    ([editorConfig, typeDefinition]) => {
      // 设置编辑器设置
      const options = JSON.parse(editorDefaultConfig) as languages.typescript.CompilerOptions;
