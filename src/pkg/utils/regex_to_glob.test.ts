 ...
 */
describe.concurrent("regexToGlob - comprehensive test suite (regrouped & commented)", () => {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("1) Literals, dots, anchors, boundaries", () => {
    it.concurrent("1.1 literal vs dot and anchors", () => {
    ...
    it.concurrent("1.2 word boundaries and non-consuming assertions", () => {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("2) Escapes and glob-reserved literals", () => {
    it.concurrent('2.1 escaped specials become literals, glob-reserved map to "?"', () => {
    ...
    it.concurrent("2.2 class-like escapes become single-char", () => {
      // 字符类逃逸统一近似为单字符匹配
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("3) Character classes", () => {
    it.concurrent("3.1 basic, negated, ranges, escapes inside", () => {
    ...
    it.concurrent("3.2 invalid classes return null", () => {
      // 不闭合或非法内容 → null
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("4) Quantifiers: *, +, ?, {m}, {m,}, {m,n} (incl. lazy/possessive)", () => {
    it.concurrent("4.1 Kleene and optional", () => {
    ...
    it.concurrent("4.2 braced exact counts", () => {
    ...
    it.concurrent("4.3 braced open upper bound", () => {
      // 下界有，上界无 → 固定下界 + '*' 近似
    ...
    it.concurrent("4.4 braced ranges", () => {
    ...
    it.concurrent("4.5 lazy variants behave same in glob", () => {
      // 惰性量词在 glob 近似中与贪婪等价
    ...
    it.concurrent("4.6 possessive variants behave same in glob", () => {
      // 占有量词对 glob 也无差别
    ...
    it.concurrent("4.7 malformed brace quantifiers should fall back to unit (not null)", () => {
      // 花括号格式错误：不应报错，回退到原文本
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("5) Groups and alternations", () => {
    it.concurrent("5.1 non-capturing groups", () => {
      // 非捕获组：量词展开或合并为 '?'
    ...
    it.concurrent("5.2 alternations without common affixes", () => {
      // 无公共前后缀：长度不等时退化为 '?' 或带 '*'
    ...
    it.concurrent("5.3 alternations with common prefix/suffix extraction", () => {
      // 有公共前/后缀：中间可变部分用 '?' 或 '*'
    ...
    it.concurrent("5.4 nested groups", () => {
      // 嵌套分组：内部分支归纳后整体近似
    ...
    it.concurrent("5.5 lookarounds mixed in groups", () => {
      // 组内混合前后查找：查找被移除
      // Lookarounds inside groups → dropped
      ok("(?:(?=x)ab|cd(?!y))", "??");
    });

    it.concurrent("5.6 alternations mixed with counted pieces (additional)", () => {
      // 分支 + 次数：按下界展开 + 近似
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("6) Canonicalization of consecutive * and ?", () => {
    it.concurrent('6.1 collapses "*?*" to "?*"', () => {
      // 组合次序变化不应影响最终规约
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("7) HTTP/URL patterns beyond examples", () => {
    it.concurrent("7.1 mixed optional and escaped segments", () => {
    ...
    it.concurrent("7.2 longer URLs with escapes (additional)", () => {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  describe.concurrent("8) Edge/odd cases", () => {
    it.concurrent("8.1 empty string and standalone operators", () => {
      // 空串应映射为空；孤立括号/方括号应失败
    ...
    it.concurrent("8.2 unbalanced parentheses", () => {
      // 括号/方括号未闭合 → null
      // Unbalanced → null
      bad("(abc");
      bad("abc)");
    ...
    it.concurrent("8.3 misc OK fallbacks and empty-alt", () => {
    ...
    it.concurrent("8.4 escaped end-of-input is invalid", () => {
      // 末尾孤立反斜杠非法
      // Trailing backslash is invalid
      bad("\\");
    });

    it.concurrent("8.5 dot + class + escapes sequence analysis", () => {
      // 混合序列应逐段近似：. → ?；类 → ?；\d+ → ?*
    ...
    it.concurrent("8.6 complex structures -> approximate (additional)", () => {
    ...
    it.concurrent("8.7 simple structures -> exact-ish (additional)", () => {
