...
import { ConsoleLogger, ConsoleTransports } from './apps/logger/console';
...
async function execScript(
	...
	return new Promise((resolve, reject) => {
		...
		SendLogger(
			LOGGER_LEVEL_INFO,
			type,
			'exec script id: ' + script.id.toString(),
			script.name,
			script.id
		);
		let execRet;
				...
				.catch((error: string, delayrun = 0) => {
					...
					if (error) {
						msg +=
							' error: ' +
							error +
							(delayrun ? ' delayrun: ' + delayrun.toString() : '');
					}
					SendLogger(LOGGER_LEVEL_ERROR, type, msg, script.name, script.id);
					if (delayrun > 0) {
...
async function disable(script: Script) {
	const context = <SandboxContext>await App.Cache.get('script:' + script.id.toString());
	if (context) {
		context.destruct();
	}
