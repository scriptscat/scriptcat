...
export interface ExternalAccessOperation {
  ...
  // kind === "source_disclosure" 时，要披露的形态（整份读 / grep + 其参数）。scripts.source.get 与
  // scripts.source.grep 共用同一个 kind、scope 与 sessionKey，批准时靠这个字段决定读整份还是跑
  // grep；请求时的幂等去重也按这个字段做，形态或参数不同就不能合并成同一个待批操作（设计 §4.2）。
  disclosure?: SourceDisclosureForm;
