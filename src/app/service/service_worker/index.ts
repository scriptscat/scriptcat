...
import { getCurrentTab, InfoNotification, sleep } from "@App/pkg/utils/utils";
import { onTabRemoved, onUrlNavigated, setOnUserActionDomainChanged } from "./url_monitor";
import { LocalStorageDAO } from "@App/app/repo/localStorage";
import { mightCreatePeriodicAlarm, setPeriodicAlarmCallback } from "./alarm";
...
export default class ServiceWorkerManager {
  ...
  initManager() {
    ...
    // 定时器处理
    setPeriodicAlarmCallback("checkScriptUpdate", async ({ isFlushed }) => {
      if (isFlushed) await sleep(9200); // 避免SW启动时所有指令一次过执行
      regularScriptUpdateCheck();
    });

    setPeriodicAlarmCallback("cloudSync", async ({ isFlushed }) => {
      if (isFlushed) await sleep(14600); // 避免SW启动时所有指令一次过执行
      // 进行一次云同步
    ...
    setPeriodicAlarmCallback("checkSubscribeUpdate", async ({ isFlushed }) => {
      if (isFlushed) await sleep(11600); // 避免SW启动时所有指令一次过执行
      subscribe.checkSubscribeUpdate();
    });

    setPeriodicAlarmCallback("checkUpdate", async ({ isFlushed }) => {
      if (isFlushed) await sleep(8400); // 避免SW启动时所有指令一次过执行
      // 检查扩展更新
      this.checkUpdate();
    });

    // 12小时检查一次扩展更新
    // 首先执行建立Alarm时会执行一次更新。2分钟延迟避免启用时马上执行
    mightCreatePeriodicAlarm("checkUpdate", {
      delayInMinutes: 2,
