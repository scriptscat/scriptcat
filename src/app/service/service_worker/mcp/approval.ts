...
import { McpOperationDAO, type McpOperation } from "@App/app/repo/mcp";
...
function autoApproveOptions(op: McpOperation): { enable?: boolean } {
  return op.kind === "install" ? { enable: true } : {};
}

/**
 * Owns the McpOperation lifecycle: every write the bridge exposes (install, enable/disable,
 ...
 * `decide(...)`, driven by an explicit human action on install.html / mcp_confirm.html — never by
...
export class McpApprovalService {
  ...
  async prepareInstall(params: {
    ...
    // code exactly like a normal install. si[1].mcp signals the bridge-triggered context (banner +
    ...
    si[1].mcp = { operationId, contentHash };
  ...
  // ---------------------------------------------------------------------------------------------
  // Confirm-page presentation. Kept separate from op creation so the policy branch in McpBridge
  ...
  private async executeSourceDisclosure(op: McpOperation): Promise<ScriptSource> {
  ...
  // Used by the human-facing approval pages (install.html / mcp_confirm.html), which are reached
  ...
  async getOperationForUI(operationId: string): Promise<McpOperation | undefined> {
