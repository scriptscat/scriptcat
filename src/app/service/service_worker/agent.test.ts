...
describe("Compact 功能", () => {
  ...
  function makeTextResponse(text: string, promptTokens = 10): Response {
    return makeSSEResponse([
  ...
  it("手动 compact：LLM 返回带 <summary> 的摘要", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(makeTextResponse(summaryText));
  ...
  it("手动 compact：带自定义指令", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(makeTextResponse("<summary>Custom summary</summary>"));
  ...
  it("自动 compact：usage 超过 80% 阈值时触发", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(makeTextResponse("<summary>Auto compacted summary</summary>"));
...
function createRunningConversation(overrides: Record<string, any> = {}) {
  return {
