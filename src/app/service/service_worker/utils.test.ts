...
describe.concurrent("compileInjectionCode", () => {
  ...
  it.concurrent(
    "script-module 脚本注入 <script type=module> 并透过 document 暴露沙盒变量（含真实授予的 GM 能力）",
    () => {
      const scriptRes = createMockScriptRes({
        metadata: { "script-module": [""], grant: ["GM.getValue", "GM.setValue"] },
      });
      const patterns = extractUrlPatterns(["@match https://example.com/*"]);
      const result = compileInjectionCode(scriptRes, scriptRes.code, patterns);

      // 仍然走普通编译路径的沙箱封装（module 包装是在沙箱之内注入），
      // 这意味着 module 拿到的 GM 与外层沙盒授予的能力（本例为 GM.getValue/GM.setValue）一致
      expect(result).toContain("with(arguments[0]||this.$)");
      expect(result).toContain("return(async function(){");
      expect(result).toContain("window['#-test-uuid']");
      ...
      // import 失败/被页面 CSP 拦截时有可观测的错误处理
  ...
  it.concurrent("script-module 搭配 @require 时不生效（module 无法访问外层词法变量）", () => {
    const scriptRes = createMockScriptRes({
      metadata: { "script-module": [""], require: ["https://example.com/lib.js"] },
    });
    const patterns = extractUrlPatterns(["@match https://example.com/*"]);
    const result = compileInjectionCode(scriptRes, scriptRes.code, patterns);
