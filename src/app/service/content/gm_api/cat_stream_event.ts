...
// 通用数据形状检查
// ============================================================================
// CAT 事件可能经由 clone 语义不同的多条传输路径到达：native 扩展消息通道确实提供
// clone 语义，但 MAIN/content 兼容路径（CustomEventMessage → parseWindowMessageBody）
// 只校验信封自身的 own-key 集合（messageId/type/data），信封内层的 data 负载原样透传，
// 不会被递归 clone 或校验。因此这里永远不能假设 message.data 已经因为"某条传输路径会
// structured clone"而安全——必须在这个共用入口先用 customClone() 主动复制一次，拒绝
// Map/Set 伪装、不可 clone 的值，并把结果规范成 null 原型或原生 Object.prototype 的
// 纯数据字典，随后只对这份 clone 做结构校验，不直接读取 raw 上的任何字段。
...
export const cloneChatStreamEvent = (raw: unknown): ChatStreamEvent | undefined => {
  ...
  switch (cloned.type) {
    ...
    case "tool_call_delta": {
      ...
      if (
        // id 允许空字符串：OpenAI/Anthropic 的后续 arguments chunk 均以 `id: tc.id || ""` 发出，
        // resolveToolCall() 本就按 id → index → 最后一个 running 的顺序回退匹配，空 id 是合法产出。
...
// buildChatStreamError() 用固定 own data property 描述符写入每个字段，而不是普通 `err.key = value`
// 赋值：字段名集合虽然已经是固定的（type/errorCode/usage/durationMs），但普通 [[Set]] 仍然会先查找
// 原型链——在 MAIN world 里，页面可以提前在 Error.prototype（或其它内建原型）上为这些名字之一放一个
// setter，那样 `err.errorCode = ...` 会调用页面的 setter 而不是在 err 上创建自有属性。用
// Native.objectDefineProperty 直接定义自有属性可以绕开这一步。descriptor 的 configurable/
// enumerable/writable 三个标志都设为 true，和原来 `Object.assign`/普通赋值产生的自有数据属性
// 完全一致，只是不再经过任何继承的 setter。
const defineErrorData = (err: Error, key: string, value: unknown): void => {
  Native.objectDefineProperty(err, key, {
    configurable: true,
    enumerable: true,
    writable: true,
    value,
...
export const buildChatStreamError = (event: ChatStreamErrorEvent): ChatStreamError => {
  ...
  defineErrorData(err, "type", "error");
  if (Native.objectHasOwn(event, "errorCode")) defineErrorData(err, "errorCode", event.errorCode);
  if (Native.objectHasOwn(event, "usage")) defineErrorData(err, "usage", event.usage);
  if (Native.objectHasOwn(event, "durationMs")) defineErrorData(err, "durationMs", event.durationMs);
  return err;
};
