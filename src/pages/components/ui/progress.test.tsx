...
describe("基础进度条组件", () => {
  it("确定进度按 value/max 计算宽度并暴露 aria 数值", () => {
    ...
    const progress = screen.getByRole("progressbar", { name: "下载进度" });
  ...
  it("不确定进度不暴露 aria 数值并使用共享动画", () => {
    ...
    const progress = screen.getByRole("progressbar", { name: "检查中" });
  ...
  it("顶部变体使用统一的细条样式", () => {
    ...
    expect(screen.getByRole("progressbar")).toHaveClass("h-0.5", "bg-primary/15");
