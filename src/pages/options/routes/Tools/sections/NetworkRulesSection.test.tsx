...
describe("Tools 网络规则摘要卡", () => {
  ...
  it("应用失败时保留已保存的规则并提供重试", async () => {
    ...
    await renderCard(clientFor(failed, { retryApply } as unknown as Partial<NetworkRuleClient>));
  ...
  it("总开关写入失败时提示且不改动本地状态", async () => {
    ...
    await renderCard(clientFor(snapshot(manyRules(3)), { setMasterEnabled } as unknown as Partial<NetworkRuleClient>));
    expect(screen.getByTestId("network-rules-summary")).toBeInTheDocument();
