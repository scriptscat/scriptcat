...
describe("exportToMarkdown", () => {
  ...
  it("应该合并 tool 角色消息到 assistant 消息", () => {
    const messages: ChatMessage[] = [
      ...
      makeMsg({
        role: "assistant",
        content: "",
        toolCalls: [{ id: "tc-1", name: "web_search", arguments: '{"query":"test"}', status: "running" }],
      }),
      makeMsg({ role: "tool", content: "搜索结果", toolCallId: "tc-1" }),
        ...
        toolCalls: [
          {
            ...
            subAgentDetails: {
              agentId: "sa-1",
              description: "搜索资料",
              messages: [{ content: "找到了相关资料", toolCalls: [] }],
            },
          },
        ],
      }),
  ...
  it("应该导出消息中的错误信息", () => {
    const messages: ChatMessage[] = [makeMsg({ role: "assistant", content: "部分内容", error: "Rate limit exceeded" })];
    const md = exportToMarkdown(baseConversation, messages);
