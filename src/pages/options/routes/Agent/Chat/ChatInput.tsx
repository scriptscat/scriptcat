...
export default function ChatInput({
  ...
  const canSend = !!(input.trim() || attachments.length > 0) && !disabled && !hasPendingMessage && !isOptimizing;
  const iconBtn =
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {isStreaming && (
                  <button
                    ...
                    onClick={onStop}
                    className="flex size-8 cursor-pointer items-center justify-center rounded-full border-none bg-warning text-warning-foreground shadow-sm transition-opacity hover:opacity-80 max-md:size-11"
                  >
