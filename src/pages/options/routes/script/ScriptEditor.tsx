...
import { IconBulb, IconDelete, IconSearch } from "@arco-design/web-react/icon";
...
type ScriptEditorContextValue = {
  // state exposed to list items
  selectSciptButtonAndTab: string;
  setSelectSciptButtonAndTab: (uuid: string) => void;
  editors: EditorItem[];
  setEditors: React.Dispatch<React.SetStateAction<EditorItem[]>>;
  hotKeys: HotKey[];
  scriptDAO: ScriptDAO;
...
const ScriptEditorContext = React.createContext<ScriptEditorContextValue | null>(null);

function useScriptEditorCtx() {
  const ctx = React.useContext(ScriptEditorContext);
  if (!ctx) {
    throw new Error("ScriptEditorContext is not available. Wrap with <ScriptEditorContext.Provider>.");
  }
  return ctx;
}
...
const ScriptEditorScriptListItem = React.memo(({ uuid, status, name }: Pick<Script, "uuid" | "status" | "name">) => {
  const {
        ...
        onClick={() => {
          ...
          if (!flag) {
            ...
            scriptCodeDAO.findByUUID(uuid).then((code) => {
              ...
              // 使用scriptDAO取最新 script 避免直接在React元件傳來傳去
              scriptDAO.get(uuid).then((script) => {
                if (!script) {
                  return;
                }
                const newEditor = {
            ...
            onOk: () => {
              scriptClient
                ...
                .then(() => {
                  ...
                  if (selectSciptButtonAndTab === uuid) {
                    setSelectSciptButtonAndTab("");
                  }
...
ScriptEditorScriptListItem.displayName = "ScriptEditorScriptListItem";
...
const ScriptEditorScriptList = React.memo(
  ({ scriptList, searchKeyword }: { scriptList: Script[]; searchKeyword: string }) => {
    ...
    return filterScriptList.map((script) => {
      return (
        <ScriptEditorScriptListItem
          key={`s_${script.uuid}`}
          uuid={script.uuid}
...
ScriptEditorScriptList.displayName = "ScriptEditorScriptList";
...
function ScriptEditor() {
  ...
  const ctxValue = React.useMemo<ScriptEditorContextValue>(
    () => ({
      selectSciptButtonAndTab,
      setSelectSciptButtonAndTab,
      editors,
  ...
  return (
    <ScriptEditorContext.Provider value={ctxValue}>
      <div
          ...
          {!hiddenScriptList && (
            ...
            >
              ...
              >
                ...
                <ScriptEditorScriptList scriptList={scriptList} searchKeyword={searchKeyword} />
              </div>
            </Col>
          )}
