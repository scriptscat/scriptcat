...
describe("安装页兼容性标记", () => {
  it("全部受支持时不产生任何标记", () => {
    ...
    expect(deriveCompatMarks(metadata, code)).toEqual({
      grants: new Map(),
      matches: new Map(),
      tags: [],
      scriptcatOnlyTags: [],
  ...
  it("标出脚本猫未实现的 @grant，并给出所在行", () => {
    ...
    expect(marks.grants).toEqual(new Map([["GM_testingAPI", 4]]));
  });
  ...
  it("代码里定位不到时仍然成条，只是没有行号——诊断不能因为缺位置而消失", () => {
    const metadata: SCMetadata = { name: ["X"], sandbox: ["raw"], grant: ["GM_testingAPI"] };
    const marks = deriveCompatMarks(metadata, "");
    expect(marks.tags).toEqual([{ tag: "sandbox", line: undefined }]);
    expect(marks.grants).toEqual(new Map([["GM_testingAPI", undefined]]));
  });
  ...
  it("解析不出规则的 @match 按取值给出行号，交给运行网站行就地替换，不另成条", () => {
    const { code, metadata } = build(`// @name X\n// @match *://a.com/*\n// @match hello-world^^\n`);
    const marks = deriveCompatMarks(metadata, code);
    expect(marks.matches).toEqual(new Map([["hello-world^^", 4]]));
    expect(marks.tags).toEqual([]);
  });
