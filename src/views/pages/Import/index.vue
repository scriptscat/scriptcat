              ...
              <template v-for="(item, index) in file.scripts">
                <v-list-item :key="'item' + index" v-if="!item.error">
                  ...
                  <v-list-item-action>
                    ...
                    <v-switch v-model="item.enable" @click.stop></v-switch>
                  </v-list-item-action>
                </v-list-item>
...
import { SCRIPT_ORIGIN_LOCAL } from "@App/model/do/script";
  ...
  async load(file: File) {
    for (let i = 0; i < file.scripts.length; i++) {
      ...
      let [newScript, oldScript] = await this.scriptCtrl.prepareScriptByCode(
        code,
        script.file_url || SCRIPT_ORIGIN_LOCAL + "://" + new Date().getTime()
      );
