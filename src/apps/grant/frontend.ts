...
export class FrontendGrant implements ScriptContext {
	...
	constructor(script: ScriptCache, browserMsg: BrowserMsg) {
		...
		const action = (action: string) => {
			return (
				...
				done: (cookie: GM_Types.Cookie[] | any, error: any | undefined) => void
			) => {
	...
	public GM_xmlhttpRequest(details: GM_Types.XHRDetails): GM_Types.AbortHandle<void> {
		...
		const handler = async () => {
			...
			if (
				details.onload &&
				(details.responseType == 'arraybuffer' || details.responseType == 'blob')
			) {
	...
	@FrontendGrant.GMFunction()
	public GM_notification(
		detail: GM_Types.NotificationDetails | string,
		ondone: GM_Types.NotificationOnDone | string,
		image?: string,
		onclick?: GM_Types.NotificationOnClick
	): void {
		...
		let click: GM_Types.NotificationOnClick,
			done: GM_Types.NotificationOnDone,
		...
		this.postRequest('GM_notification', [data], (grant: Grant) => {
			const data =
				<{ type: string; id: string; index: number; user: boolean }>grant.data || {};
			switch (data.type) {
				case 'click': {
	...
	protected valueChangeListener = new Map<
		number,
		{ name: string; listener: GM_Types.ValueChangeListener }
	>();
	...
	ValueChange = (name: string, value: Value): void => {
		this.valueChangeListener.forEach((val) => {
			if (val.name === name) {
				const old =
					this.script.value && this.script.value[name] && this.script.value[name].value;
	...
	public GM_registerMenuCommand(name: string, listener: () => any, accessKey?: string): number {
		...
		this.postRequest(
			...
			[{ name: name, accessKey: accessKey, id: id }],
			(grant: Grant) => {
	...
	@FrontendGrant.GMFunction()
	protected GM_getCookieStore(
		tabid: number,
		done: (storeId: number, error: any | undefined) => void
	): void {
...
export class SandboxContext extends FrontendGrant {
	...
	public valueChange = (msg: any) => {
		...
		if (
			this.script.metadata['storagename'] &&
			this.script.metadata['storagename'][0] == model.storageName
		) {
