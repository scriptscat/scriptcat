...
(async function () {
  ...
  const { describe, check, expect, run } = SCTest.create({ name: "GM API 完整测试(同步)" });
  ...
  describe("GM Cookie API", () => {
    ...
    check(
      "自动断言",
      "GM_cookie - 回调风格 set (带 domain 和 path)",
      () => {
        return new Promise((resolve, reject) => {
          GM_cookie(
            "set",
