...
// @grant        window.focus
// @grant        unsafeWindow
// @require      https://cdn.jsdelivr.net/gh/scriptscat/scriptcat@9e0b465196d1b87786a8cb4d5a0c42fed148b735/example/tests/lib/sctest.js
// @run-at       document-end
// ==/UserScript==
...
(async function () {
  ...
  const { describe, check, expect, run } = SCTest.create({ name: "半沙盒环境测试" });
  ...
  function assertSame(expected, actual, message) {
    if (!Object.is(expected, actual)) {
      throw new Error(`${message} - 期望 ${formatValue(expected)}, 实际 ${formatValue(actual)}`);
    }
  }
  ...
  function assertThrowsOrKeepsValue(assign, read, expected, message) {
    ...
    assertSame(expected, read(), `${message}${threw ? "（赋值抛出，值保持不变）" : ""}`);
  }
  ...
  describe("沙盒全局身份", () => {
    ...
    check(
      "自动断言",
      "window/self/globalThis/top/parent/frames 均指向沙盒对象",
      () => {
        expect(typeof unsafeWindow).toBe("object");
        assertNotSame(unsafeWindow, window, "默认 grant 环境下 window 不应是页面 window");
    ...
    check(
      "自动断言",
      "沙盒 window 使用空原型，但保留页面 Window 外观 (Issue #962)",
      () => {
    ...
    check(
      "自动断言",
      "页面 DOM named property 不应穿透为沙盒全局变量 (Issue #273, #700)",
      () =>
        withCleanup(
          () => {
    ...
    check(
      "自动断言",
      "删除沙盒全局变量不应删除页面同名全局变量 (Issue #522)",
      () =>
        withCleanup(
          () => {
    ...
    check(
      "自动断言",
      "Object.prototype 污染不会穿透到沙盒 window",
      () =>
        withCleanup(
          () => {
    ...
    check(
      "自动断言",
      "console 与页面 console 隔离，但方法可用",
      () => {
        assertNotSame(unsafeWindow.console, console, "沙盒 console 应与页面 console 不是同一个对象");
  ...
  describe("原生函数与事件代理", () => {
    check(
      "自动断言",
      "裸调用原生函数已绑定真实页面 window，避免 Illegal invocation (Issue #189)",
      async () => {
    ...
    check(
      "自动断言",
      "取出 window.addEventListener 后调用不会 Illegal invocation (Issue #773)",
      () =>
        withCleanup(
          () => {
    ...
    check(
      "自动断言",
      "被 Proxy 包装的原生函数仍可安全裸调用 (Issue #1030)",
      async () => {
    ...
    check(
      "自动断言",
      "getter 返回页面 window 时会替换为沙盒 window (Issue #1427)",
      () => {
        expect(self).toBe(window);
        expect(parent).toBe(window);
        expect(top).toBe(window);
    ...
    check(
      "自动断言",
      "onxxx 函数赋值由页面事件触发，event.target 为 unsafeWindow",
      () =>
        withCleanup(
          () => {
            let count = 0;
    ...
    check(
      "自动断言",
      "onxxx 普通对象只保存不注册监听，primitive 值应移除已注册的监听",
      () =>
        withCleanup(
          async () => {
            let handled = false;
    ...
    check(
      "自动断言",
      "TM半沙盒：把祖先类别继承直接写在半沙盒上 (Issue #1462 PR #1463)",
      async () => {
  ...
  describe("GM API 注入与命名空间", () => {
    check(
      "自动断言",
      "GM_info、GM.info 与 unsafeWindow 正确暴露",
      () => {
    ...
    check(
      "自动断言",
      "GM_setValue/GM_getValue/GM_deleteValue 同步路径正常",
      () =>
        withCleanup(
          () => {
    ...
    check(
      "自动断言",
      "GM.setValue/GM.getValue/GM.deleteValue Promise 路径正常",
      async () =>
        withCleanup(
          async () => {
            ...
            expect((await withTimeout(GM.listValues(), "GM.listValues")).includes(key)).toBeTruthy();
            await withTimeout(GM.deleteValue(key), "GM.deleteValue");
            expect(await withTimeout(GM.getValue(key, "fallback"), "GM.getValue fallback")).toBe("fallback");
          },
          () => {
    ...
    check(
      "自动断言",
      "GM_setValues/GM_getValues 以及 GM.getValues 依赖注入正常",
      () =>
        withCleanup(
          async () => {
            ...
            const asyncPicked = await withTimeout(GM.getValues({ [keyB]: null }), "GM.getValues");
    ...
    check(
      "自动断言",
      "GM_addStyle 与 GM.addStyle 都插入页面 document",
      async () =>
        withCleanup(
          async () => {
            const className = `${markerPrefix}_style`;
            const style = GM_addStyle(`.${className} { color: rgb(1, 2, 3) !important; }`);
            expect(style.tagName).toBe("STYLE");
            expect(style.ownerDocument).toBe(document);

            const asyncStyle = await withTimeout(
              GM.addStyle(`.${className}_async { color: rgb(4, 5, 6) !important; }`),
              "GM.addStyle"
            );
          ...
          () => {
            document.querySelectorAll("style[data-scriptcat-sandbox-test]").forEach((node) => node.remove());
          }
        ),
      null,
      null,
    ...
    check(
      "自动断言",
      "GM_addElement 支持默认 parent、显式 parent、非字符串 property",
      async () =>
        withCleanup(
          async () => {
    ...
    check(
      "自动断言",
      "window.close/window.focus grant 暴露为沙盒 window 方法",
      () => {
