...
(async ({ escapeHtml, fmtMs, logLine, pass, fail, skip, setCounts, setStatus, setQueue, prettyStack, state, assert, assertTrue, assertDeepEq, resPrint, isFirefox, printSummary, showResultPanel }) => {
  ...
  const basicTests = [
    {
      ...
      async run(fetch) {
        ...
        assert(res.responseText, decodedBase64, "responseText ok");
        assert(res.response, decodedBase64, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
...

    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(res.responseText, decodedBase64, "responseText ok");
        assert(res.response, undefined, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(res.responseText, decodedBase64, "responseText ok");
        assert(res.response instanceof XMLDocument, true, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(res.responseText, undefined, "responseText ok");
        assert(res.response instanceof ReadableStream, true, "response ok");
        assert(res.responseXML, undefined, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(res.responseText, decodedBase64, "responseText ok");
        assert(res.response instanceof ArrayBuffer, true, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(res.responseText, decodedBase64, "responseText ok");
        assert(res.response instanceof Blob, true, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(`${res.responseText}`.includes('"code": 200'), true, "responseText ok");
        assert(`${res.response}`.includes('"code": 200'), true, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(`${res.responseText}`.includes('"code": 200'), true, "responseText ok");
        assert(typeof res.response === "object" && res.response?.code === 200, true, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(res.responseText?.length >= 8 && res.responseText?.length <= 32, true, "responseText ok");
        assert(res.response, res.responseText, "response ok");
        assert(res.responseXML instanceof XMLDocument, true, "responseXML ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(q.x, "1", "query args");
        const hdrs = body.headers || {};
        assert(hdrs["X-Custom"] || hdrs["x-custom"], "Hello", "custom header echo");
        assert(res.finalUrl, url, "finalUrl matches");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status after redirect is 200");
        assert(res.finalUrl, target, "finalUrl is redirected target");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        try {
          ...
          assert(e?.kind, "error", "error ok");
          assert(e?.res?.status, 408, "statusCode ok");
          assert(!e?.res?.finalUrl, true, "!finalUrl ok");
          assert(e?.res?.responseHeaders, "", "responseHeaders ok");
          assert(objectProps(e?.res), "ok", "Object Props OK");
        }
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res?.status, 301, "status is 301");
        assert(res?.finalUrl, url, "finalUrl is original url");
        assert(typeof res?.responseHeaders === "string" && res?.responseHeaders !== "", true, "responseHeaders ok");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert((body.form || {}).a, "1", "form a");
        assert((body.form || {}).b, "two", "form b");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        const body = JSON.parse(res.responseText);
        assert(res.status, 200);
        assertTrue(body.data && body.data.length > 0, "server received some data");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assertTrue(res.response instanceof ArrayBuffer, "arraybuffer present");
        assert(res.response.byteLength, size, "byte length matches");
        assertTrue(progressCounter >= 1, "progressCounter >= 1");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assertTrue(res.response instanceof Blob, "blob present");
        const buf = await res.response.arrayBuffer();
        assert(buf.byteLength, size, "byte length matches");
        assertTrue(progressCounter >= 1, "progressCounter >= 1");
        assert(objectProps(res), "ok", "Object Props OK");
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assertTrue(res.response && typeof res.response === "object", "parsed JSON object");
        assertTrue(res.response.origin, "has JSON fields");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assertTrue(res.response instanceof Document, "xml present");
        assertTrue(res.responseXML !== null, "xml OK");
        assertTrue(!!res.responseXML.querySelector("test-123"), "xml content ok");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assertTrue(res.response instanceof Document, "xml present");
        assertTrue(res.responseXML !== null, "xml OK");
        assertTrue(!!res.responseXML.querySelector("parsererror"), "xml content ok");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assertTrue(typeof res.responseText === "string" && res.responseText.length > 0, "responseText available");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        try {
          ...
          assert(e.kind, "timeout", "timeout path taken");
        }
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(normal.response.status, 200, "normal onloadend status 200");
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assertTrue(progressEvents >= 4, "received at least 4 progress events");
        // `progress` is guaranteed to fire only in the Fetch API.
        assertTrue(fetch ? lastLoaded > 0 : lastLoaded >= 0, "progress loaded captured");
        assertTrue(!response, "no response");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        // `progress` is guaranteed to fire only in the Fetch API.
        assertTrue(fetch ? lastLoaded > 0 : lastLoaded >= 0, "progress loaded captured");
        assertTrue(response instanceof ReadableStream && typeof response.getReader === "function", "response");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assertTrue((res.responseText || "")?.length > 0, "body for HEAD");
        assertTrue(typeof res.responseHeaders === "string", "response headers present");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        assert(res.responseText || "", "", "no body for HEAD");
        assertTrue(typeof res.responseHeaders === "string", "response headers present");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assertTrue(res.status === 200 || res.status === 204, "200/204 on OPTIONS");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        const body = JSON.parse(res.responseText);
        assert(body.method, "DELETE", "server saw DELETE");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(cookieABC, "123", "cookie abc=123");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assertTrue(!`${cookies}`.includes("abc=123"), "no Cookie header when anonymous");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        const body = JSON.parse(res.responseText);
        const cookies = body.headers.Cookie || body.headers.cookie;
        assertTrue(`${cookies}`.includes("abc=123"), "Cookie header");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        const body = JSON.parse(res.responseText);
        const cookies = body.headers.Cookie || body.headers.cookie;
        assertTrue(!cookies, "no Cookie header when anonymous");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200);
        const body = JSON.parse(res.responseText);
        assert(body.authenticated, true, "authenticated true");
        assert(body.user, "user", "user echoed");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 418, "418 I'm a teapot");
        assert(objectProps(res), "ok", "Object Props OK");
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assertTrue([200, 405].includes(res.status), "200 or 405 depending on server handling");
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
    {
      ...
      async run(fetch) {
        ...
        try {
          ...
          assert(e.kind, "error", "onerror path taken");
          assertTrue(e.res, "e.res exists");
          assert(e.res.status, 0, "status 0");
          assert(e.res.statusText, "", 'statusText ""');
          assert(e.res.finalUrl, undefined, "finalUrl undefined");
          assert(e.res.readyState, 4, "readyState DONE");
          assert(!e.res.response, true, "!response ok");
          assert(e.res.responseText, "", 'responseText ""');
          assert(e.res.responseXML, undefined, "responseXML undefined");
          assert(typeof (e.res.error || undefined), "string", "error set");
          assert(
    ...
    },
    {
      ...
      async run(fetch) {
        try {
          ...
          assert(e.res.readyState, 4, "readyState 4");
          assert(objectProps(e.res), "ok", "Object Props OK");
        }
      },
    },
    {
      ...
      async run(fetch) {
        try {
          ...
          assert(e.kind, "abort", "abort path taken");
        }
      },
    },
    {
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        if (!fetch) {
          assert(progressCount >= 2, true, "progressCount ok");
          assertDeepEq(
            resultList,
            [
              "onreadystatechange 1.000;r=missing;t=missing;x=missing",
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(headers.get("content-type"), "application/json; charset=utf-8", "content-type ok");
        assert(
        ...
        assert(headers.get("cross-origin-opener-policy"), "same-origin", "cross-origin-opener-policy ok");
        assert(headers.get("content-encoding") !== "deflate", true, "content-encoding ok");
      },
    },
    {
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        assert(res.status, 200, "status 200");
        assert(typeof res.responseHeaders, "string", "responseHeaders is string");
        assert(res.responseHeaders.trim() === res.responseHeaders, true, "no extra space");
        // Each line should end with \r\n
        const lines = res.responseHeaders.split("\r\n");
        for (let i = 0; i < lines.length - 1; i++) {
          assertTrue(lines[i].length > 0, `header line ${i} present`);
        }
        assert(objectProps(res), "ok", "Object Props OK");
      },
    },
  ...
  // 跟测试对象无关的基础设施：DOM 构建、日志面板、计数器、断言函数、堆栈美化打印。
  ...
  // lines use (<b>, <pre style="...">, entities) into real DOM nodes — no
  ...
  // ---------- Panel ----------
  // Builds the whole test-runner panel DOM tree and injects it into the page.
  // Wiring the #start button (which needs runAll, a suite-specific function)
  // is left to the caller — only the parts that don't depend on this file's
  // specific test suite live here.
  ...
  return { escapeHtml, fmtMs, logLine, pass, fail, skip, setCounts, setStatus, setQueue, prettyStack, state, assert, assertTrue, assertDeepEq, resPrint, isFirefox, printSummary, showResultPanel };
})());
