...
(async function () {
  ...
  const { describe, check, expect, run } = SCTest.create({ name: "GM_xhr 重定向测试" });
  ...
  const basicTests = [
    {
      ...
      async run(fetch) {
        const { res } = await gmRequest({
          method: "GET",
          url: `${HB}/get?testing=234&abc=567`,
          responseType: "json",
          fetch,
        });
    ...
    },
    {
      ...
      async run(fetch) {
        const { res } = await gmRequest({
          method: "GET",
          url: `${HB}/get?abc=567&testing=234`,
          responseType: "json",
          fetch,
        });
        expect(res.status).toBe(200);
    ...
    },
    {
      ...
      async run(fetch) {
        ...
        const { res } = await gmRequest({
          method: "GET",
          url: `${HB}/redirect-to?url=${encodeURIComponent(target)}`,
          fetch,
        });
        expect(res.status).toBe(200);
      ...
      async run(fetch) {
        try {
          await Promise.race([
            gmRequest({
              method: "GET",
              url: `${HB}/redirect-to?url=${encodeURIComponent(`${HB}/get?z=96`)}`,
              redirect: "error",
              fetch,
            }),
  ...
  const tests = [...basicTests, ...basicTests.map((t) => ({ ...t, useFetch: true }))];
