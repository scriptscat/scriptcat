...
ScriptCat（脚本猫）是一个参考油猴设计理念的用户脚本管理器，也提供具备扩展 API 的后台脚本运行框架。内置代码编辑器支持智能补全和语法检查，帮助提高脚本开发效率。
...
- **油猴脚本兼容性**：兼容情况取决于脚本使用的 API 和元数据，部分脚本可能需要调整
...
3. **兼容性**：兼容情况取决于脚本使用的 API 和元数据。如果脚本无法运行，欢迎通过
