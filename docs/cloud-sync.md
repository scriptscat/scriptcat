...
当前所有 provider 的 capabilities 均为 false（见「provider 差异」），因此生产路径实际只传 `modifiedDate`；条件参数逻辑保留并有单测覆盖，供后续"按 endpoint 探测能力"方案回开时使用。

`pushScript()` 成功后返回本地计算的 md5 patch，仅用于 provider list 暂时看不到刚上传文件时兜底。它不能覆盖 provider 已返回的原生 digest。

#### 自我 412 收敛

`pushScript()` 捕获到 `.user.js` 写入 conflict、且本轮尚未重试（`selfHealRetried` 未置位）时，调用 `rebaseSelfWrittenConflict()`：读 `sync_content_md5` 取本机上次同步的内容 md5，再 `fs.list()` + 读云端当前内容比对。
...
| --- | --- | --- | --- | --- | --- |
| WebDAV | `etag` | 不声明 | 不声明 | 不声明 | 条件头映射代码保留（`If-Match` / `overwrite: false`）但同步层不会触发 |
| S3 | `ETag` 去引号 | 不声明 | 不声明 | 不声明 | 条件头映射代码保留（`if-match` / `if-none-match: *`）但同步层不会触发 |
| OneDrive | `eTag` | 不声明 | 不声明 | 不声明 | 条件头映射代码保留（simple PUT / upload session `If-Match`）但同步层不会触发 |
...
**为什么 WebDAV / S3 / OneDrive 不再声明条件能力**（#1504 真实服务端实测结论）：

- WebDAV：两台真实服务端行为相反且都不可用——坚果云对**正确 etag** 的 `If-Match` 也恒 412（CAS 路径完全写不进），Alist 类完全忽略 `If-Match`/`If-None-Match`（条件头是静默 no-op，无任何防覆盖）。没有客户端改动能同时适配两者。
- S3：MinIO 实测仅 `If-Match` CAS 真原子；`If-None-Match: *` 被忽略（视版本），DeleteObject 本无条件删除语义。
- OneDrive（Graph）：三项实测都生效，但恰恰因为它合规，历史上 `scriptInstall` 的 create-only 误用会 409/412 卡死每次编辑——能力按 provider 类型硬编码的假设整体不成立。

结论：capability 不能按协议类型硬编码，只能按具体账号/endpoint 实测探测。在探测机制落地前，所有 provider 保守声明为不支持，同步层回落无条件读写 + 内容基线方向判定 + 冲突停走通知（最终一致，不会卡死）。provider 侧条件头映射代码与单测保留，供后续回开。

### WebDAV

维护时注意：

- `capabilities` 三项均为 false（原因见上表说明）。
- 条件头映射代码保留：ETag 写入和删除时 `quoteETag()` 补齐引号（坚果云 PROPFIND 的 etag 不带引号且只认不带引号的 `If-Match`，是能力关闭的原因之一）。
...
- 条件头映射代码保留：写入 `expectedDigest` 时发送 `if-match`；create-only 发送 `if-none-match: *`；删除 `expectedDigest` 时发送 `if-match`。
