...
- **Never hard-code a color value** — see [`develop.md` § UI](../develop.md#ui). For dark-only tweaks use the `dark:` variant.
- Use one token per semantic color concept. Reuse an existing token; add one only for a new concept with light and dark values, and document its role here.
