...
`matches()`/`querySelector()` call, which is measurably slower at TSX-suite scale. To quantify the win, measure
before/after in one environment with the JSON-report method below.
