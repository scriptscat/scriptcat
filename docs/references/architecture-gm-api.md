...
- **Content side** ([`src/app/service/content/gm_api/`](../../src/app/service/content/gm_api)) — what runs *near*
  ...
  (`GM_xmlhttpRequest`, `GM_setValue`). Built on `GM_Base`, which owns the request facade. `USER_SCRIPT` and
  supported MAIN calls use the native extension channel; the MAIN page-bridge fallback validates execution handles
  and grants before forwarding. Brokered calls from early scripts wait for their page binding; synchronous DOM
  helpers and declared resource getters remain available during early startup through their local paths.
...
- **Values** flow through `ValueService`. Supported MAIN scripts receive per-document updates over the native
  extension channel; the page bridge does not carry their value updates.
