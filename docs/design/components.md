...
| `StateScreen` | `ui/state-screen.tsx` | Full-area loading / empty / error / success screen with `tone`, monospace detail box & progress slot ([state patterns](./patterns.md#state-patterns)) |
...
| `FormField` / `SwitchField` | `ui/form-field.tsx` | Labeled field wrapper (label + description + error + required) / switch row ([forms & validation](./patterns.md#forms--validation)) |
| `TooltipIconButton` | `ui/tooltip-icon-button.tsx` | Icon button with tooltip + loading state |
| `NameAvatar` | `components/NameAvatar.tsx` | Name-seeded rounded icon (script / subscribe / provider) — see [stored-value type badges](./tokens.md#stored-value-type-badges-string--number--boolean--object) |
...
- **Feedback:** transient → `toast`; persistent / in-page → see [state patterns](./patterns.md#state-patterns).
