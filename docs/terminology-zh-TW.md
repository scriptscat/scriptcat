...
4. 技術名詞以台灣開發者常用寫法為準；例如 `正規表達式` 應保留，不改為 `正規運算式`。
...
| `表達式` | 一般 expression 可依語境用 `運算式` | `value_export_expression`, `cookie_export_expression`, `cron_invalid_expr`, `scheduled_script_description_description_expr`, `expression_format_error` | `正規表達式` 是台灣既定譯法，必須保留。 |
| `監聽檔案` / `停止監聽` | `監看檔案` / `停止監看` | `watch_file_description`, `watch_file`, `stop_watch_file` | 開發語境中的 `事件監聽` 可保留；面向使用者的檔案功能以 `監看` 較直觀。 |
...
4. 維持既有 placeholder、HTML/React 標記與 i18next 插值格式，不因術語修正而改壞行為。
5. 交付前重新搜尋本次新增或修改的繁體中文，確認未引入本文件標示應避免的用詞。
