...
**Уязвимости безопасности — исключение: не создавайте публичный Issue.** Сообщайте о них приватно согласно нашей [Политике безопасности](../SECURITY.md), чтобы исправление можно было подготовить до публичного раскрытия.
...
```bash
pnpm run dev
# Сборка для разработки без source maps
pnpm run dev:noMap
```
