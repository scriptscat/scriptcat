...
本書は、ScriptCat の日本語（`ja-JP`）UI およびドキュメントで使用する用語の基準です。日本語の文言を翻訳または修正する際は、日本語として自然で、製品 UI として一貫した表現を使用します。
...
4. placeholder、HTML/React タグ、i18next の補間形式、`@grant` などの識別子は変更しません。
...
| **B. UI 操作の標準表現** | ボタン、メニュー、状態表示などで現在採用されている表現です。 |
...
| 概念 | 使用する表記 | 現行用例 key |
| --- | --- | --- |
| create | `新しい...を作成` / `作成` | `create_script`, `create_background_script`, `create_success_note` |
| save / save as | `保存` / `名前を付けて保存` | `save`, `save_as`, `save_success` |
| import / export | `インポート` / `エクスポート` | `import`, `export`, `import_file`, `export_file` |
| install | `インストール` | `install_script`, `install_success`, `install_failed` |
| run / runtime | `実行` / `ランタイム` | `run`, `running`, `runtime`, `script_run_at.title` |
| enable / disable | `有効` / `無効`、操作文では `有効にする` | `enable`, `disable`, `enable_script`, `enable_background.title` |
| settings | `設定` | `settings`, `script_setting.title`, `editor_config` |
| permission | `権限` / `許可` | `permission`, `permission_management`, `allow_once` |
| connection | `接続` | `connect`, `connection_success`, `sync_system_connect_failed` |
| synchronization | `同期` | `script_sync`, `sync_status`, `guide_setting_sync_title` |
| restore | `復元` | `restore`, `restore_default_values` |
| load / reload | `読み込み` / `再読み込み` | `loading`, `install_page_loading`, `click_to_reload` |
| tabs | `タブ` | `close_current_tab`, `script_run_env.all`, `script_run_env.normal-tabs` |
| interface display | `表示` / `非表示` | `badge_type_none`, `editor.show_script_list`, `editor.hide_script_list` |
...
2. 同じ機能の既存文言がある場合は、「製品・機能の標準用語」と「UI 操作の標準表現」を優先してください。
